# Siamese U-Net (concat) by Rodrigo Caye Daudt
...
class SiamUnet_conc(nn.Module):
    ...
    def _make_encoder_layers(self):
        ...
        for i in range(4):
            ...
            bn = self.batchnorm(self.num_channels[i])
            relu = nn.ReLU()
            dropout = nn.Dropout2d(p=0.2)
            encoder_layer.extend([conv1,bn,relu,dropout,conv2,bn,relu,dropout])
            if i >= 2: encoder_layer.extend([conv2,bn,relu,dropout])
    ...
    def _make_decoder_layers(self):
        ...
        for i in range(4):
            decoder_layer=[]
            deconv1 = self.deconv(self.num_channels[i+1]+self.num_channels[i], self.num_channels[i])
            if i >= 2: deconv2 = self.conv(self.num_channels[i], self.num_channels[i])
            ...
            bn2 = self.batchnorm(self.out_channels) if i == 0 else self.batchnorm(self.num_channels[i-1])
            relu = nn.ReLU()
            dropout = nn.Dropout2d(p=0.2)
            if i >= 2: decoder_layer.extend([deconv1,bn1,relu,dropout,deconv2,bn1,relu,dropout,deconv3,bn2,relu,dropout])
            else: decoder_layer.extend([deconv1,bn1,relu,dropout,deconv3,bn2,relu,dropout])
    ...
    def forward(self, x1, x2):
        ...
        x4d = torch.cat((pad4(x4d), x4_1, x4_2), 1)
        ...
        x3d = torch.cat((pad3(x3d), x3_1, x3_2), 1)
        ...
        x2d = torch.cat((pad2(x2d), x2_1, x2_2), 1)
        ...
        x1d = torch.cat((pad1(x1d), x1_1, x1_2), 1)
...
if __name__ == '__main__':
    ...
    siamunet_cat=SiamUnet_conc(3,10)
    print(siamunet_cat(input1, input2).shape)
    print('okk!')