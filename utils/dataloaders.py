...
def full_path_loader(data_dir):
    ...
    for img in train_data:
        train_label_paths.append(data_dir + 'train/C/' + img)
    for img in valid_data:
        val_label_paths.append(data_dir + 'val/C/' + img)
...
def cdd_loader(img_path, label_path, aug):
    ...
    img1 = Image.fromarray(tifffile.imread(dir + 'A/' + name))
    img2 = Image.fromarray(tifffile.imread(dir + 'B/' + name))
    label = Image.fromarray(tifffile.imread(label_path))
